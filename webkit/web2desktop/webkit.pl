...
$mw->set_title("GtkPerl Mini Webkit");
$mw->set_default_size( '800', '600' );
$mw->signal_connect( 'delete_event' => sub { Gtk2->quit; } );
...
$url_entry->set_text('http://');

$main_table->attach( $back_button, 0, 1, 0, 1, [ 'expand', 'fill' ],
    'expand', 10, 10 );

$main_table->attach( $forward_button, 1, 2, 0, 1, [ 'expand', 'fill' ],
    'expand', 10, 10 );

$main_table->attach( $reload_button, 2, 3, 0, 1, [ 'expand', 'fill' ],
    'expand', 10, 10 );

$main_table->attach( $url_entry, 3, 4, 0, 1, [ 'expand', 'fill' ],
    'expand', 10, 10 );

$main_table->attach(
    $scroll_window, 0, 4, 1, 2,
    [ 'expand', 'fill' ],
    [ 'expand', 'fill' ],
    10, 10
);
